 ...
 * COMRADE_MDWE=OFF leaves it out of a build that runs under valgrind, which
 * maps its own shadow memory executable and dies under it. Irreversible. */
...
static int stage_etc(const char *root)
{
	...
	if (dir_is_shared(rdir)) {
		/* A source gone by now was rewritten, not unstageable. */
